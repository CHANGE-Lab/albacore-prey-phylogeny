...
p3_6 <- gheatmap(p2_6, my_prey_traits[ ,c(4,10)], offset=0.01, width=0.10,font.size=2,
                 colnames_angle=-85, colnames_offset_y = 4.5, colnames = F) +
  scale_fill_viridis_d(name = "Physical Defense", option = "B",
                       breaks = c("0","1"),
                       limits = c("0","1"))
...
p5_6 <- gheatmap(p4_6, trophic_level, offset=0.11, width=0.05, font.size=2,
  ...
  theme(legend.key.size = unit(3,'mm'),
        ...
        legend.spacing = unit(0.02,'cm'),
        legend.position = c(0.99,0.5)) +
  annotate('text', x = 1.17, y = 5.5, label = 'Trophic Level', angle = -85, size = 2)+
  annotate('text', x = 1.12, y = 5.8, label = 'Physical Defence', angle = -85, size = 2)+
  annotate('text', x = 1.07, y = 5.5, label = 'Body Shape', angle = -85, size = 2)
